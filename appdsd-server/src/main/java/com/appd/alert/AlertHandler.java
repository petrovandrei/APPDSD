                ...
                if (info != null) {
                    ...
                    if (previousMessageTime != null && (messageDate.getTime() - previousMessageTime.getTime()) <= sensorConfiguration.getCheckFrequency()) {
                        ...
                        if (sensorState == SensorState.WARNING) {
                            ...
                            // Ulterieurement il y a eu des messages d'alerte de ce capteur
                            Timestamp dangerTime = info.getFirstDangerMessageDate();
                            // intervention sur le capteur effectuée
                            if (numberOfWarning >= warningMessagesNeeded) {
                                System.err.println(" /!\\ Sensor n°" + sensorId + " has been taken care of /!\\");
                            ...
                            // Reset les info du cache ( l'alerte et la date du premier message d'alerte)
                            info.reset();
                        } else {
                    ...
                    // Case : cache vide ou le dernier message d'alerte est trop ancien
                    else {
                        if (sensorState == SensorState.WARNING) {
                            // Set le nombre de messages d'alerte envoyés par le capteur
                    ...
                    log.info("The sensor n°" + sensorId + " stoped sending messages for a while");
                }
                //dernier message d'alerte envoyé par le capteur
                info.setLastMessageDate(messageDate);
                // mise à jour du cache
                cacheInfoBySensor.put(sensorId, info);
                // affiche les information du capteur
